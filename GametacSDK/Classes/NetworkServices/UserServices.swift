...
public class UserServices: NSObject {
    ...
    func refreshToken(completion:@escaping(MessageStruct, UserStruct)->()){
        let param:[String:Any] = ["refresh_token":NSAuthentication.sharedInstance.getRefreshToken(),
                                  ...
                                  "client_secret":GametacSDK.sharedInstance.client_secret]
    ...
    func getUserInfo(completion:@escaping(MessageStruct, UserStruct)->()){
        ...
        NSNetRequest.sharedInstance.postWithHeader(url: self.kAPIGetUserInfo, param: ["client_id":GametacSDK.sharedInstance.client_id], userToken: false) { (json, msg) in
            debugPrint(json)
