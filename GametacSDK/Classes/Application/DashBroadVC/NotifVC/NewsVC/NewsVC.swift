...
class NewsVC: UIViewController, IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "Tin tức")
    }

    @IBOutlet weak var tbvListNews: UITableView!
    
    
    func configTableView(){
        tbvListNews.delegate = self
        tbvListNews.dataSource = self
        //        tbvListReport.emptyDataSetDelegate = self
        //        tbvListReport.emptyDataSetSource = self
        //        tbvListReport.separatorColor = UIColor.clear
        self.tbvListNews.register(UINib(nibName: "NotiCell", bundle: nil), forCellReuseIdentifier: "NotiCell")
