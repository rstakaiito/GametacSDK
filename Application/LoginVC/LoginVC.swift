...
class LoginVC: UIViewController, GIDSignInDelegate/*, GIDSignInDelegate*/ {
    ...
    func playWithoutLogin(){
        UserServices.sharedInstance.playWithoutLogin(device_id: deviceToken ?? "",
                                                     ...
                                                     client_secret: SDKServices.sharedInstance.client_secret) { (msg, userInfo) in
                                                        switch msg.code {
                                                        case 200:
        ...
        fbLoginManager.logIn(permissions: ["public_profile","email"], from: self) { (result, err) in
            if (err == nil) {
                ...
                if result?.grantedPermissions != nil {
                    UserServices.sharedInstance.loginSocial(token: AccessToken.current?.tokenString ?? "", provider: "facebook", client_id: SDKServices.sharedInstance.client_id, client_secret: SDKServices.sharedInstance.client_secret, completion: { (msg, userProfile) in
                        switch msg.code {
                        case 200:
                            print(userProfile)
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
        if let error = error {
            ...
            UserServices.sharedInstance.loginSocial(token: access_token ?? "", provider: "google", client_id: SDKServices.sharedInstance.client_id, client_secret: SDKServices.sharedInstance.client_secret, completion: { (msg, userProfile) in
                switch msg.code {
                case 200:
                    print(userProfile)
                    NSAuthentication.sharedInstance.profile = userProfile
